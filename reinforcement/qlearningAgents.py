...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        possible_actions = list(filter(lambda action: value == self.getQValue(state, action), legal_actions))
    ...
    def update(self, state, action, next_state, reward):
        ...
        """
        key = (state, action)
        new_value = self.discount * self.computeValueFromQValues(next_state) + reward
        old_value = self.getQValue(state, action)
        self.values[key] = old_value + (new_value - old_value) * self.alpha
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, next_state, reward):
        ...
        max_value = self.getValue(next_state)
        difference = (reward + self.discount * max_value) - q_value
